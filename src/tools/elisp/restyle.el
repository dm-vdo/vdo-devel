...
(defun reflow-function-decls-in-buffer ()
  (let ((result)
	(fill-column reflow-fill-column))
...
(defun run-test ()
  (message "%S" (reflow-function-decls-in-buffer)))
...
(defun restyle-declarations-in-files ()
  (apply-buffer-op-to-argv-files #'reflow-function-decls-in-buffer))
...
(defun reflow-function-calls-in-buffer ()
  (let ((fill-column reflow-fill-column))
    (font-lock-fontify-region (point-min) (point-max))
    (goto-char (point-min))
    (while (find-next-function-body)
      (save-restriction
	(narrow-to-defun)
	;; Look for "foo(arglist)" or "(ptr->callback)(arglist)".
...
(defun restyle-files ()
  (apply-buffer-op-to-argv-files
   (lambda ()
     (reflow-function-calls-in-buffer)
     (reflow-function-decls-in-buffer))))

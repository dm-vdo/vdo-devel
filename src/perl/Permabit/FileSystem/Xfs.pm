...
sub _ensureXfsErrorLevelConfigured {
  ...
  if (!$machine->{_xfs_error_level_configured}) {
    ...
    # Only register cleanup if we captured a value; restore on cleanup; log failures, don’t croak
    if ($originalErrorLevel ne '') {
      ...
      $log->warn("Could not determine original fs.xfs.error_level; skipping cleanup restore");
    }

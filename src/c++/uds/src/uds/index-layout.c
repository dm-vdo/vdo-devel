						     ...
						     struct region_table **table_ptr)
{
	...
	result = vdo_allocate_flex(region_count, regions,
						 ...
						 struct region_table **table_ptr)
{
	...
	result = vdo_allocate_flex(region_count, regions, "layout region table",
				   &table);
	if (result != VDO_SUCCESS)
		return result;
...
static int create_index_layout(struct index_layout *layout, struct uds_configuration *config)
{
	...
	result = vdo_allocate(sizes.save_count, __func__,
			      &layout->index.saves);
	if (result != VDO_SUCCESS)
		return result;
					  ...
					  struct region_table **table_ptr)
{
	...
	result = vdo_allocate_flex(header.region_count, regions,
					    ...
					    struct region_table *table, u64 first_block)
{
	...
	result = vdo_allocate(layout->super.max_saves,

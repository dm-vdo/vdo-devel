			       ...
			       struct chapter_writer **writer_ptr)
{
	...
	result = vdo_allocate_extended(struct chapter_writer, index->zone_count, chapters,
		   ...
		   struct uds_index **new_index)
{
	...
	result = vdo_allocate_extended(struct uds_index, config->zone_count, zone_queues,
				       "index", &index);
	if (result != VDO_SUCCESS)
		return result;

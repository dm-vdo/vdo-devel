 ...
 * - (and so on).
 */
static void compute_coding_constants(u32 mean_delta, u16 *min_bits,
				     u32 *min_keys, u32 *incr_keys)
{
	/*
...
static int initialize_delta_zone(struct delta_zone *delta_zone, size_t size,
				 ...
				 u32 mean_delta, u32 payload_bits, u8 tag)
{
	int result;
			       ...
			       size_t memory_size, u8 tag)
{
	...
	result = UDS_ALLOCATE(zone_count, struct delta_zone,
			      "Delta Index Zones", &delta_index->delta_zones);
	if (result != UDS_SUCCESS)
		return result;
	...
	for (z = 0; z < zone_count; z++) {
		...
		result = initialize_delta_zone(&delta_index->delta_zones[z],
					       zone_memory, first_list_in_zone,
					       lists_in_zone, mean_delta,
					       payload_bits, tag);
		if (result != UDS_SUCCESS) {
			uds_uninitialize_delta_index(delta_index);
			return result;
...
static bool verify_delta_index_page(u64 nonce, u16 list_count,
				    u64 expected_nonce, u8 *memory,
				    size_t memory_size)
{
	unsigned int i;
	...
	for (i = 0; i < list_count; i++)
		if (get_immutable_start(memory,
					i) > get_immutable_start(memory,
								 i + 1))
			return false;
				    ...
				    size_t memory_size)
{
	...
	if (!verify_delta_index_page(nonce, list_count, expected_nonce, memory,
		...
		if (!verify_delta_index_page(nonce, list_count, expected_nonce,
					     memory, memory_size))
			/*
 ...
 */
static void move_bits_down(const u8 *from, u64 from_offset, u8 *to,
			   u64 to_offset, u32 size)
{
	const u8 *source;
	u8 *destination;
	u8 offset;
 ...
 */
STATIC void move_bits(const u8 *from, u64 from_offset, u8 *to, u64 to_offset,
		      u32 size)
{
	u64 field;
...
int uds_pack_delta_index_page(const struct delta_index *delta_index,
			      u64 header_nonce, u8 *memory, size_t memory_size,
				      ...
				      const u8 *data)
{
	...
	move_bits(data, save_info->bit_offset, delta_zone->memory,
		  delta_list->start, delta_list->size);
	return UDS_SUCCESS;
}
				   ...
				   u8 *data)
{
	...
	result = uds_read_from_buffered_reader(buffered_reader, buffer,
					       sizeof(buffer));
	if (result != UDS_SUCCESS)
		return uds_log_warning_strerror(result,
...
int uds_write_guard_delta_list(struct buffered_writer *buffered_writer)
{
	...
	result = uds_write_to_buffered_writer(buffered_writer, buffer,
					      sizeof(buffer));
	if (result != UDS_SUCCESS)
		uds_log_warning_strerror(result,
				 ...
				 struct delta_index_entry *delta_entry)
{
	...
	result = ASSERT((list_number < delta_index->list_count),
			...
			list_number, delta_index->list_count);
	if (result != UDS_SUCCESS)
		return UDS_CORRUPT_DATA;
	...
	if (delta_index->mutable) {
		...
		delta_list->start = get_immutable_start(delta_zone->memory,
							list_number);
...
int uds_get_delta_entry_collision(const struct delta_index_entry *delta_entry,
				  u8 *name)
{
	int result;
...
int uds_set_delta_entry_value(const struct delta_index_entry *delta_entry,
			      u32 value)
{
	int result;
			     ...
			     size_t growing_size)
{
	...
	/* Compute the new offsets of the delta lists. */
	compute_new_list_offsets(delta_zone, growing_index, growing_size,
				 used_space);
...
static void encode_delta(const struct delta_index_entry *delta_entry)
{
	...
	if (delta_entry->delta < delta_zone->min_keys) {
		set_field(delta_entry->delta, memory, offset,
			  delta_zone->min_bits);
		return;
	}
 ...
 * be provided.
 */
int uds_put_delta_index_entry(struct delta_index_entry *delta_entry, u32 key,
			      u32 value, const u8 *name)
{
	int result;
	struct delta_zone *delta_zone;
...
size_t uds_compute_delta_index_size(u32 entry_count, u32 mean_delta,
				    u32 payload_bits)
{
	u16 min_bits;
	u32 incr_keys;
...
u32 uds_get_delta_index_page_count(u32 entry_count, u32 list_count,
				   u32 mean_delta, u32 payload_bits,

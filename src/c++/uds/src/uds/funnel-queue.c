...
static struct funnel_queue_entry *get_oldest(struct funnel_queue *queue)
{
	 ...
	 * funnel_queue_put() between its caller setting up the entry and making it visible.
	 */
 ...
 * the funnel_queue_poll() function, but the queue will not be considered idle.
 */

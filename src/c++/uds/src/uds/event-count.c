...
static const unsigned int ONE_WAITER = 1;
/* value used to increment the event counter */
static const unsigned int ONE_EVENT = (1 << 16);
/* bit mask to access the waiters field */
static const unsigned int WAITERS_MASK = (ONE_EVENT - 1);
/* bit mask to access the event counter */
static const unsigned int EVENTS_MASK = ~WAITERS_MASK;
...
struct event_count {
	...
	/* # of signal() calls */
	atomic64_t signals;
	/* # of uds_release_semaphore() calls */
	atomic64_t wakeups;
	/* # of event_count_wait() calls */
	atomic64_t waits;
	/* # of uds_acquire_semaphore() calls */
	atomic64_t sleeps;
	/* # of sem_timedwait() timeouts */
	atomic64_t timeouts;
...
#if defined(TEST_INTERNAL) && defined(INSTRUMENTED)
	atomic64_add(waiters, &count->wakeups);

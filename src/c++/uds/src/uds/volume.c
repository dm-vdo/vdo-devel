					      ...
					      unsigned int zone_count)
{
	...
	result = vdo_allocate(VOLUME_CACHE_MAX_QUEUED_READS,
			      "volume read queue", &cache->read_queue);
	if (result != VDO_SUCCESS)
		return result;

	result = vdo_allocate(cache->zone_count,
			      "Volume Cache Zones", &cache->search_pending_counters);
	if (result != VDO_SUCCESS)
		return result;

	result = vdo_allocate(cache->indexable_pages, "page cache index",
			      &cache->index);
	if (result != VDO_SUCCESS)
		return result;

	result = vdo_allocate(cache->cache_slots, "page cache cache",
			      &cache->cache);
	if (result != VDO_SUCCESS)
		return result;
		    ...
		    struct volume **new_volume)
{
	...
	result = vdo_allocate(config->read_threads, "reader threads",
...
void uds_free_volume(struct volume *volume)
{
	...
	mutex_destroy(&volume->read_threads_mutex);
#endif /* __KERNEL__ */

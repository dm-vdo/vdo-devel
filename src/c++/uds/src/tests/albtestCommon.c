...
{
  CU_SuiteInfo *s;
  UDS_ASSERT_SUCCESS(vdo_allocate(1, CU_SuiteInfo, "suite", &s));
  *s = *suite;
...
TestResult runSuite(const CU_SuiteInfo *suite)
{
  ...
  UDS_ASSERT_SUCCESS(vdo_allocate(numTests, TestResult, "Test Results", &result.sub));
...
TestResult runSuites(const CU_SuiteInfo *suites)
{
  ...
  UDS_ASSERT_SUCCESS(vdo_allocate(numSuites, TestResult, "Suite Results", &result.sub));

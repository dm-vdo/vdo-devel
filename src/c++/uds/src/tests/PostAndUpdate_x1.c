               ...
               void *data __attribute__((unused)))
{
  ...
  if (type == UDS_POST) {
    ...
    funnel_queue_put(testQueue, &tbc->queueEntry);
...
static void updateBlockNames(void *argument)
{
  ...
  for (counter = 0; counter < numBlocksInTest; ) {
    ...
    while ((fqe = funnel_queue_poll(testQueue)) == NULL) {
      event_token_t token = event_count_prepare(testEvent);
      if ((fqe = funnel_queue_poll(testQueue)) != NULL) {
...
static void postAndUpdate(void)
{
  ...
  UDS_ASSERT_SUCCESS(make_funnel_queue(&testQueue));

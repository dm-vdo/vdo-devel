...
static void uncompressRandomData(const char* source, int isize, int osize)
{
  ...
  VDO_ASSERT_SUCCESS(vdo_allocate(frameSize, __func__, &frame));
...
static void compressString(const char *source)
{
  ...
  VDO_ASSERT_SUCCESS(vdo_allocate(sourceLen, __func__, &compressed));
  VDO_ASSERT_SUCCESS(vdo_allocate(sourceLen + 1, __func__, &copy));
...
static int compressBlockFromStream(FILE *stream, int sourceLen)
{
  ...
  VDO_ASSERT_SUCCESS(vdo_allocate(sourceLen, __func__, &compressed));
  VDO_ASSERT_SUCCESS(vdo_allocate(sourceLen, __func__, &copy));
  VDO_ASSERT_SUCCESS(vdo_allocate(LZ4_context_size(), __func__, &ctx));
  VDO_ASSERT_SUCCESS(vdo_allocate(sourceLen, __func__, &source));

...
{
  CU_ASSERT_EQUAL(vdo_waitq_num_waiters(&zone->flush_waiters), 3);
  struct tree_page *treePage
    = container_of(vdo_waitq_get_first_waiter(&zone->flush_waiters),
...
static void countTreeWaiters(void)
{
  if (vdo_get_callback_thread_id() == zone->thread_id) {
    size_t waiters = vdo_waitq_num_waiters(&zone->flush_waiters);

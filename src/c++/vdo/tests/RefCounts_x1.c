...
/**********************************************************************/
static Wrapper *asWrapper(struct vdo_completion *wrapperCompletion)
{
  STATIC_ASSERT(offsetof(Wrapper, completion) == 0);
  return ((Wrapper *) wrapperCompletion);
}

/**********************************************************************/
static void didAcquireVIO(struct vdo_waiter *element, void *context)
{
  Wrapper *wrapper = container_of(element, Wrapper, waiter);
...
{
  Wrapper *wrapper = asWrapper(wrapperCompletion);
  acquire_vio_from_pool(pool, &wrapper->waiter);
  vdo_finish_completion(wrapperCompletion);
}

/**********************************************************************/
static void acquireVIO(Wrapper *wrapper)
{
...
/**********************************************************************/
static void initWrapper(Wrapper *wrapper)
{
...
{
  int count_per_slab = vdo->depot->slabs[0]->reference_block_count / BLOCKS_PER_VIO;
  int i;
  ...
  VDO_ASSERT_SUCCESS(vdo_allocate(vio_count, Wrapper, __func__, &wrappers));

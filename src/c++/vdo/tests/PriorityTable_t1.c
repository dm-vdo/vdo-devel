...
{
  VDO_ASSERT_SUCCESS(make_priority_table(MAX_PRIORITY, &table));
}

/**********************************************************************/
static void tearDown(void)
{
  free_priority_table(UDS_FORGET(table));
}
...
{
  priority_table_enqueue(table, entry->priority, &entry->priorityNode);
}
...
static void drainTable(size_t entryCount)
{
  ...
  for (size_t i = 0; i < entryCount; i++) {
    QueueEntry *entry = (QueueEntry *) priority_table_dequeue(table);
    CU_ASSERT_PTR_NOT_NULL(entry);
    CU_ASSERT_TRUE(entry->priority <= lastPriority);
    lastPriority = entry->priority;
  }

  CU_ASSERT_PTR_NULL(priority_table_dequeue(table));
  CU_ASSERT_TRUE(is_priority_table_empty(table));
}
...
static void testSingletonTable(void)
{
  ...
  enqueue(&entry);
  CU_ASSERT_FALSE(is_priority_table_empty(table));

  // Dequeue it.
  CU_ASSERT_PTR_EQUAL(&entry, priority_table_dequeue(table));
  ...
  enqueue(&entry);
  priority_table_remove(table, &entry.priorityNode);
  drainTable(0);
...
static void testRandomTable(void)
{
  ...
  for (size_t i = 0; i < COUNT; i += 2) {
    priority_table_remove(table, &entries[i].priorityNode);
  }

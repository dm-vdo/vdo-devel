...
static void printVDOSizeInfo(struct vdoInfo *vdoInfo)
{
  printf("Blocksize: %d\n", VDO_BLOCK_SIZE);
  printf("VDO:\n");
  ...
  printf("  Sparse: %d\n", vdoInfo->sparseIndex);
  printf("  Slab Size: %d\n", vdoInfo->slabBlockCount);
  printf("  Slab Count: %ld\n", vdoInfo->slabCount);
  printf("  Index Memory: %s\n", vdoInfo->indexMemorySize);
}

static void printMetaDataInfo(struct vdoInfo *vdoInfo)
{
  ...
  printf("  UDS Index Size: %ldMB\n", (vdoInfo->udsIndexSize *
				       VDO_BLOCK_SIZE) / MB);
  printf("  Total BlockMap Pages: %ldMB\n",
          (vdoInfo->totalBlockMapPages * VDO_BLOCK_SIZE) / MB);
  printf("  Slab Reference Count Usage: %ldMB\n",
          (vdo_get_saved_reference_count_size(vdoInfo->userDataBlocks) *
	   VDO_BLOCK_SIZE) / MB);
  printf("  Slab Jouranl Usage: %ldMB\n",
          (vdoInfo->slabCount * DEFAULT_VDO_SLAB_JOURNAL_SIZE *
	   VDO_BLOCK_SIZE) / MB);
}

static void printUsage(struct vdoInfo *vdoInfo)
{
  printf("VDO Usage:\n");
  printf("  Physical Size: %ldGB\n", vdoInfo->physicalSize / GB);
  printf("  Logical Size: %ldGB\n", vdoInfo->logicalSize / GB);
  printf("  Usable Size: %ldGB\n", vdoInfo->totalUsableSpace / GB);
  printf("  Dedup Window: %ldGB\n", getDedupeWindowSize(vdoInfo->indexMemorySize));
}

static void printMemoryUsage(struct vdoInfo *vdoInfo)
{
  printf("In Memory Usage:\n");
  printf("  Block Map Cache: %ldMB\n",
           (vdoInfo->blockMapCacheSize * VDO_BLOCK_SIZE) / MB);
  printf("  Forest Memory Usage: %ldMB\n",
           (vdoInfo->totalForestMemoryUsage * VDO_BLOCK_SIZE) / MB);
  printf("  Slab Reference Count Usage: %ldMB\n",
	   (vdo_get_saved_reference_count_size(vdoInfo->userDataBlocks) *
	    VDO_BLOCK_SIZE) / MB);
}
...
{
  printVDOSizeInfo(vdoInfo);
  printMetaDataInfo(vdoInfo);
  printUsage(vdoInfo);
  printMemoryUsage(vdoInfo);
}
...
{
  int result;
  enum { OPT_BLOCKMAPCACHE_SIZE = 'A', OPT_INDEXMEMORY_SIZE, OPT_LOGICAL_SIZE,
	 OPT_PHYSICAL_SIZE, OPT_SLABBITS, OPT_SPARSEINDEX, OPT_THREADCOUNT};
  struct option options[] = {
    ...
    {"threadCount",       required_argument, NULL, OPT_THREADCOUNT},
    {NULL,                0,                 NULL, 0},
  };
  int opt;
  ...
  while ((opt = getopt_long_only(argc, argv, "", options, NULL)) != -1) {
    ...
    case OPT_THREADCOUNT:
      result = parseInt(optarg, &vdoInfo->threadCount);
      break;
    default:
      usage();
      break;

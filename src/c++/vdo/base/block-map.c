...
static int make_forest(struct block_map *map, block_count_t entries)
{
	...
	result = vdo_allocate_flex(struct forest, map->root_count, trees,
				   __func__, &forest);
	if (result != VDO_SUCCESS)
		return result;
			 ...
			 struct vdo_completion *completion)
{
	...
	result = vdo_allocate_flex(struct cursors, map->root_count, cursors,
				   __func__, &cursors);
	if (result != VDO_SUCCESS) {
						  ...
						  block_count_t maximum_age)
{
	...
	result = vdo_allocate_flex(struct dirty_lists, maximum_age, eras,
			 ...
			 struct block_map **map_ptr)
{
	...
	result = vdo_allocate_flex(struct block_map,

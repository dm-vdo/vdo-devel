...
static int __must_check allocate_cache_components(struct vdo_page_cache *cache)
{
	...
	result = vdo_allocate(cache->page_count, "page infos",
			      &cache->infos);
	if (result != VDO_SUCCESS)
		return result;
			...
			struct boundary *new_boundary, struct forest *forest)
{
	...
	result = vdo_allocate(forest->segments,
...
static int make_forest(struct block_map *map, block_count_t entries)
{
	...
	result = vdo_allocate_flex(map->root_count, trees, __func__, &forest);
	if (result != VDO_SUCCESS)
		return result;
			 ...
			 struct vdo_completion *completion)
{
	...
	result = vdo_allocate_flex(map->root_count, cursors, __func__, &cursors);
	if (result != VDO_SUCCESS) {
						  ...
						  block_count_t maximum_age)
{
	...
	result = vdo_allocate_flex(maximum_age, eras, __func__, &zone->dirty_lists);
	if (result != VDO_SUCCESS)
		return result;
			 ...
			 struct block_map **map_ptr)
{
	...
	result = vdo_allocate_flex(vdo->thread_config.logical_zone_count,
				   zones, __func__, &map);
	if (result != VDO_SUCCESS)
		return result;

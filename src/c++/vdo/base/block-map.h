...
static inline struct block_map_page * __must_check vdo_as_block_map_page(struct tree_page *tree_page)
{
...
int __must_check vdo_decode_block_map(struct block_map_state_2_0 state,
				      ...
				      struct vdo *vdo,
				      struct recovery_journal *journal,
				      nonce_t nonce, page_count_t cache_size,
				      block_count_t maximum_age,
...
void vdo_resume_block_map(struct block_map *map,
			  struct vdo_completion *parent);
...
void set_info_state(struct page_info *info,

...
#ifdef INTERNAL
void update_slab_summary_entry(struct vdo_slab *slab, struct vdo_waiter *waiter,
...
void encode_slab_journal_entry(struct slab_journal_block_header *tail_header,
			       slab_journal_payload *payload, slab_block_number sbn,
...
void launch_reference_block_write(struct vdo_waiter *waiter, void *context);
...
bool __must_check find_free_block(const struct vdo_slab *slab,
...
int __must_check make_slab(physical_block_number_t slab_origin,
			   struct block_allocator *allocator, slab_count_t slab_number,
			   bool is_new, struct vdo_slab **slab_ptr);

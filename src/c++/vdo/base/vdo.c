						 ...
						 struct thread_config *config)
{
	...
	result = vdo_allocate(config->logical_zone_count,
	...
	result = vdo_allocate(config->physical_zone_count,
	...
	result = vdo_allocate(config->hash_zone_count,
	...
	result = vdo_allocate(config->bio_thread_count,
	     ...
	     struct vdo **vdo_ptr)
{
	...
	*vdo_ptr = vdo;
	...
	snprintf(vdo->thread_name_prefix, sizeof(vdo->thread_name_prefix),
		 "vdo%u", instance);
	result = vdo_allocate(vdo->thread_config.thread_count, __func__,
			      &vdo->threads);
	if (result != VDO_SUCCESS) {
				    ...
				    thread_id_t thread_id)
{
	...
	result = vdo_allocate(1, __func__,
			      &read_only_listener);
	if (result != VDO_SUCCESS)
		return result;

 ...
 *                         device. Does not validate the data.
...
/**
 * load_geometry_block() - Synchronously read the geometry block from a vdo's underlying block
...
/**
 * zero_blocks() - Synchronously write zeroes to a vdo's underlying block device.
...
/**
 * vdo_is_formatted() - Check to see whether we should format or not.
 *
 * In order to determine whether we should format or not, we read the volume geometry of
 * the VDO device. The volume geometry contains the key layout information for VDO, so we
 * use this single block as the decider. If the block is all zeroes, we will format the
 * device. If it is anything else, we will not. This is similar to how dm-thin works.
 *
 * @vdo        The vdo to format.
			  ...
			  unsigned int instance, char **reason)
{
	...
	if (formatted == 0) {
		/* Get size of future indexer */
		block_count_t index_blocks;

		result = compute_index_blocks(config, &index_blocks);
		if (result != VDO_SUCCESS) {
			*reason = "Unable to calculate index size";
			return VDO_BAD_CONFIGURATION;
		}
		...
		if (necessary_size > config->physical_blocks) {
			...
			*reason = "Not enough space to make a VDO";
			return VDO_NO_SPACE;
		}

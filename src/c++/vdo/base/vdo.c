...
static int write_format_layout(struct vdo *vdo, char **error_ptr)
{
	...
	if (result != VDO_SUCCESS) {
		*error_ptr = "cannot write super block";
		return result;
	}

	result = save_geometry_block(vdo);
	if (result != VDO_SUCCESS)
		*error_ptr = "cannot write geometry block";
...
static int initialize_component_states(struct vdo *vdo, nonce_t nonce, char **error_ptr)
{
	...
	if (result != VDO_SUCCESS) {
		...
		*error_ptr = "failed to initialize slab config";
		return result;
	}
...
static int initialize_format_layout(struct vdo *vdo, char **error_ptr)
{
	...
	if (result != VDO_SUCCESS) {
		*error_ptr = "could not initialize volume geometry";
		return result;
	}

	result = initialize_component_states(vdo, nonce, error_ptr);
	if (result != VDO_SUCCESS) {
		*error_ptr = "could not initialize components";
		return result;
	}
	...
	if (result != VDO_SUCCESS)
		*error_ptr = "validatation of compoenents failed";
...
static int initialize_super_block(struct vdo *vdo, struct vdo_super_block *super_block)
{
	...
	if (vdo->super_block.buffer != NULL)
		return VDO_SUCCESS;

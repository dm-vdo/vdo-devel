 ...
 * DOC: VDO Layout.
 *
 * vdo_layout is an object which manages the layout of a VDO. It wraps fixed_layout, but includes
 * the knowledge of exactly which partitions a VDO is expected to have. Because of this knowledge,
 * the vdo_layout validates the fixed_layout encoded in the super block at load time, obviating the
 * need for subsequent error checking when other modules need to get partitions from the layout.
 *
 * The vdo_layout also manages the preparation and growth of the layout for grow physical
 * operations.
 */
...
/*
 * A fixed layout is like a traditional disk partitioning scheme. In the beginning there is one
 * large unused area, of which parts are carved off. Each carved off section has its own internal
 * offset and size.
 */
struct fixed_layout;
struct partition;
...
block_count_t __must_check vdo_get_total_fixed_layout_size(const struct fixed_layout *layout);
...
block_count_t __must_check vdo_get_fixed_layout_partition_size(const struct partition *partition);
...
size_t __must_check vdo_get_fixed_layout_encoded_size(const struct fixed_layout *layout);

int __must_check vdo_encode_fixed_layout(const struct fixed_layout *layout, struct buffer *buffer);

int __must_check vdo_decode_fixed_layout(struct buffer *buffer, struct fixed_layout **layout_ptr);
...
int __must_check
vdo_decode_layout(struct fixed_layout *layout, struct vdo_layout **vdo_layout_ptr);
...
block_count_t __must_check vdo_get_next_layout_size(struct vdo_layout *vdo_layout);
...
#endif // defined(INTERNAL) || defined(__KERNEL__)

struct fixed_layout * __must_check vdo_get_fixed_layout(const struct vdo_layout *vdo_layout);

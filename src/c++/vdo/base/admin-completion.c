...
struct admin_completion *vdo_admin_completion_from_sub_task(struct vdo_completion *completion)
{
...
{
	thread_id_t expected = admin_completion->get_thread_id(admin_completion);
...
void vdo_initialize_admin_completion(struct vdo *vdo, struct admin_completion *admin_completion)
{
	...
	vdo_initialize_completion(&admin_completion->completion, vdo, VDO_ADMIN_COMPLETION);
...
struct vdo_completion *vdo_reset_admin_sub_task(struct vdo_completion *completion)
{
	...
	completion->callback_thread_id = admin_completion->get_thread_id(admin_completion);
...
/* Prepare the sub-task completion to run on the same thread as its enclosing completion. */
...
static void admin_operation_callback(struct vdo_completion *vdo_completion)
{
	...
	completion = container_of(vdo_completion, struct admin_completion, completion);
 ...
 * should not be called from base threads unless it is certain the calling thread won't be needed
 * to perform the operation. It may (and should) be called from non-base threads.
 *

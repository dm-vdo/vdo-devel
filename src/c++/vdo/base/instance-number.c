...
static int grow_bit_array(void)
{
	unsigned int new_count =
		max(bit_count + BIT_COUNT_INCREMENT, (unsigned int) BIT_COUNT_MINIMUM);
	unsigned long *new_words;
	int result;

	result = uds_reallocate_memory(words,
...
static int vdo_allocate_instance_locked(unsigned int *instance_ptr)
{
	...
	if (instance >= bit_count) {
		...
		result = ASSERT(instance < bit_count, "impossibly, no zero bit found");
		if (result != UDS_SUCCESS)
			return result;
	}
...
/**
 * vdo_initialize_instance_number_tracking() - Initialize the instance-number tracking data
 *                                             structures.
 */
...
/*
 * vdo_clean_up_instance_number_tracking() - Free up the instance-number tracking data structures.
 */

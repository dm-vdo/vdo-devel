...
static int process_fill_message(struct dedupe_index *index)
{
	unsigned long seed;
	bool can_fill;
	...
	spin_lock(&index->state_lock);
	can_fill = (!index->suspended && (index->index_state == IS_OPENED));
	spin_unlock(&index->state_lock);
	if (!can_fill) {
		return -EINVAL;
	}
	...
	for (;;) {
		...
		for (i = 0; i < 1000; i++) {
			...
			request->session = index->index_session;
		...
		result = uds_flush_index_session(index->index_session);
		if (result != UDS_SUCCESS) {
			uds_log_error_strerror(result,
...
int vdo_message_dedupe_index(struct dedupe_index *index, const char *name)
{
	if (strcasecmp(name, "index-close") == 0) {
		...
		return process_fill_message(index);
#endif /* VDO_INTERNAL */
	}
	return -EINVAL;
}

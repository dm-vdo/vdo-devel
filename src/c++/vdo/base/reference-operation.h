...
/**
 * typedef pbn_lock_getter - Get the pbn_lock associated with a reference_operation.
 * @operation: The reference_operation.
 *
 * Return: The pbn_lock on the block of a reference_operation or NULL if there isn't one.
 */
typedef struct pbn_lock *pbn_lock_getter(struct reference_operation operation);

/*
 * The current operation on a physical block (from the point of view of the data_vio doing the
 * operation)
 */
struct reference_operation {
...
/**
 * vdo_get_reference_operation_pbn_lock() - Get the pbn_lock associated with the current
 *                                          reference_operation.
 * @operation: The reference operation.
 *
 * Return: The pbn_lock on the block of the current operation or NULL if there isn't one.
 */
static inline struct pbn_lock * __must_check
vdo_get_reference_operation_pbn_lock(struct reference_operation operation)
{
	return ((operation.lock_getter == NULL) ? NULL : operation.lock_getter(operation));
}

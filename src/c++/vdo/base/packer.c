...
static int __must_check make_bin(struct packer *packer)
{
	...
	result = vdo_allocate_extended(struct packer_bin, VDO_MAX_COMPRESSION_SLOTS, incoming,
				       __func__, &bin);
...
int vdo_make_packer(struct vdo *vdo, block_count_t bin_count, struct packer **packer_ptr)
{
	 ...
	 */
	result = vdo_allocate_extended(struct packer_bin, MAXIMUM_VDO_USER_VIOS / 2, incoming,

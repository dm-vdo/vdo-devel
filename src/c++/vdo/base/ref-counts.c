...
#ifdef INTERNAL
/**
 * pbn_to_index() - Convert a block number to the index of a reference counter
 *                  for that block.
 * @ref_counts: The reference counts object.
 ...
 *
 * Return: A success or error code, specifically: VDO_OUT_OF_RANGE if the pbn
 *         is out of range.
 */
...
/**
 * vdo_are_equivalent_ref_counts() - Check whether two reference counters are
 *                                   equivalent.
 ...
 */
bool vdo_are_equivalent_ref_counts(struct ref_counts *counter_a,
...
#ifdef INTERNAL
/**
 * vdo_count_unreferenced_blocks() - Count all unreferenced blocks in a range
 *                                   [start_block, end_block) of physical
 *                                   block numbers.
 * @ref_counts: The reference counters to scan.
 * @start_pbn: The physical block number at which to start scanning (included
 *             in the scan).
 * @end_pbn: The physical block number at which to stop scanning (excluded
 *           from the scan).
 *
...
#ifdef INTERNAL
/**
 * clear_dirty_reference_blocks() - A waiter_callback to clean dirty reference
 *                                  blocks when resetting.

			...
			struct vdo_work_queue **queue_ptr)
{
	...
	result = vdo_allocate(1, "round-robin work queue",
			      &queue);
	if (result != VDO_SUCCESS)
		return result;

	result = vdo_allocate(thread_count,
...
static struct simple_work_queue *get_current_thread_work_queue(void)
{
	 ...
	 *
	 * So we check for the null pointer first, on older kernels. The kthread code initially
	 * overloaded the set_child_tid field to use for its pointer in PF_KTHREAD processes. (If
	 * PF_KTHREAD is clear, kthread_func will return null anyway so we needn't worry about that
	 * case.)
	 *
	 * This bug was fixed in the 5.13 kernel release, but the 5.17 kernel release changed the
	 * task structure field used such that this workaround will break things on newer kernels.
	 * It shows up as a null pointer returned for the current work queue even when running in
	 * the work queue thread.
	 *
	 * Any backports of the 5.13 fix to custom pre-5.13 kernels should have no problem with
	 * this. Backports of the 5.17 change to 5.13 and later should be okay with this #if check;
	 * backports to pre-5.13 will need further protection.
	 */

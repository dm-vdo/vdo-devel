...
/*
 * A read_only_notifier is responsible for propogating the fact that the VDO has encountered an
 * unrecoverable error to all base threads. It also persists the read-only state to the super
 * block.
 *
 * The notifier also provides the ability to wait for any notifications to be complete in order to
 * not cause super block write races when shutting down the VDO.
 */
...
int __must_check vdo_make_read_only_notifier(bool is_read_only,
...
void vdo_enter_read_only_mode(struct read_only_notifier *notifier, int error_code);

bool __must_check vdo_is_read_only(struct read_only_notifier *notifier);

bool __must_check vdo_is_or_will_be_read_only(struct read_only_notifier *notifier);

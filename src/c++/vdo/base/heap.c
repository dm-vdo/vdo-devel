...
static void sift_heap_down(struct heap *heap, size_t top_node, size_t last_node)
{
	...
	while ((left_child = (2 * top_node)) <= last_node) {
		...
		if (left_child < last_node) {
			...
			if (heap->comparator(&heap->array[left_child], &heap->array[right_child])
			    < 0)
				swap_node = right_child;
		}

		/*
		 * Stop sifting if top_node is at least as large as its largest child, which means
		 * the heap invariant was restored by the previous swap.
		 */
		if (heap->comparator(&heap->array[top_node], &heap->array[swap_node]) >= 0)
			return;
		...
		/*
		 * Descend into the sub-heap rooted at that child, going around the loop again in
		 * place of a tail-recursive call to sift_heap_down().
		 */
		top_node = swap_node;
	}

	/*
	 * We sifted the element all the way to a leaf node of the heap, so the heap invariant has
	 * now been restored.
	 */
}
...
void build_heap(struct heap *heap, size_t count)
{
	...
	/*
	 * All the leaf nodes are trivially valid sub-heaps. Starting with the parent of the
	 * right-most leaf node, restore the heap invariant in that sub-heap by sifting the top
	 * node of the sub-heap down into one of its children's valid sub-heaps (or not, if the top
	 * node is already larger than its children). Continue iterating through all the interior
	 * nodes in the heap, in sort of a reverse breadth-first traversal, restoring the heap
	 * invariant for each (increasingly larger) sub-heap until we reach the root of the heap.
	 * Once we sift the root node down into one of its two valid children, the entire heap must
	 * be valid, by induction.
	 *
	 * Even though we operate on every node and potentially perform an O(log N) traversal for
	 * each node, the combined probabilities of actually needing to do a swap and the heights
	 * of the sub-heaps sum to a constant, so restoring a heap from the bottom-up like this has
	 * only O(N) complexity.
	 */
...
bool pop_max_heap_element(struct heap *heap, void *element_ptr)
{
	...
	/*
	 * Move the right-most leaf node to the vacated root node, reducing the number of elements
	 * by one and violating the heap invariant.
	 */
	if (root_node != last_node)
		memcpy(&heap->array[root_node], &heap->array[last_node], heap->element_size);
	heap->count -= 1;
...
static inline size_t sift_and_sort(struct heap *heap, size_t root_node, size_t last_node)
{
	/*
	 * We have a valid heap, so the largest unsorted element is now at the top of the heap.
	 * That element belongs at the start of the partially-sorted array, preceding all the
	 * larger elements that we've already removed from the heap. Swap that largest unsorted
	 * element with the right-most leaf node in the heap, moving it to its sorted position in
	 * the array.
	 */
	heap->swapper(&heap->array[root_node], &heap->array[last_node]);
	/*
	 * The sorted list is now one element larger and valid. The heap is one element smaller,
	 * and invalid.
	 */
	last_node -= heap->element_size;
	/* Restore the heap invariant by sifting the swapped element back down into the heap. */
	sift_heap_down(heap, root_node, last_node);
	return last_node;
}
 ...
 * repeatedly popping the maximum element off the heap and moving it to the spot vacated at the end
 * of the heap array. When the function returns, the heap will be empty and the array will contain
 * the elements in sorted order, from heap minimum to heap maximum. The sort is unstable--relative
 * ordering of equal keys is not preserved. This operation has O(N*log2(N)) time complexity.
 *
...
size_t sort_heap(struct heap *heap)
{
	...
	/*
	 * All zero-length records are identical and therefore already sorted, as are empty or
	 * singleton arrays.
	 */
	if ((heap->count < 2) || (heap->element_size == 0))
		return heap->count;
	...
	/*
	 * Get the byte array offset of the root node, and the right-most leaf node in the 1-based
	 * array of records that will form the heap.
	 */
	root_node = (heap->element_size * 1);
	last_node = (heap->element_size * heap->count);

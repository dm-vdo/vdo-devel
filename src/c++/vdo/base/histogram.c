...
static void histogram_show_histogram(struct histogram *h, char **buf, unsigned int *maxlen)
{
	...
	histogram_write_item(buf, maxlen, "%s", "\"buckets\" : { ");
	...
	for (i = 0; i <= max; i++) {
		...
		if (h->log_flag) {
			if (i == h->num_buckets) {
				histogram_write_item(buf, maxlen, "%s", "\"Bigger\"");
			} else {
	...
	histogram_write_item(buf, maxlen, "%s", " }, ");
}
...
static void histogram_show_mean(struct histogram *h, char **buf, unsigned int *maxlen)
{
	...
	if (count == 0) {
		histogram_write_item(buf, maxlen, "%s", "\"mean\" : 0.0, ");
		return;
	}
...
static void histogram_show_log_flag(struct histogram *h, char **buf, unsigned int *maxlen)
{
	...
	histogram_write_item(buf, maxlen, "\"log flag\" : \"%d\" ", h->log_flag);
}
...
{
	histogram_write_item(buf, maxlen, "\"%s\" : { ", histogram->name);

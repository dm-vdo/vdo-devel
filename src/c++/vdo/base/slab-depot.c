...
static unsigned int count_valid_references(vdo_refcount_t *counters)
{
	...
	while (words_left > 0) {
		...
		unsigned int words_left_this_iteration = min_t(unsigned int, words_left,
							       max_words_per_iteration);

		words_left -= words_left_this_iteration;

		while (words_left_this_iteration--) {
			u64 word = *words;
			u64 temp;
		...
		/*
		 * This use of modulus to add the bytes together only works if the sum is kept
		 * below 255. Hence the 254 limit above.
		 */

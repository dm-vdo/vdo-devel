...
static void complete_reaping(struct vdo_completion *completion)
{
	...
	return_vio_to_pool(vio_as_pooled_vio(as_vio(vdo_forget(completion))));
...
static void complete_write(struct vdo_completion *completion)
{
	...
	return_vio_to_pool(vdo_forget(pooled));
...
static void finish_reference_block_load(struct vdo_completion *completion)
{
	...
	struct reference_block *block = completion->parent;
	struct vdo_slab *slab = block->slab;
	size_t num_blocks = vio->io_size / VDO_BLOCK_SIZE;
	size_t i;
	char *data = vio->data;

	for (i = 0; i < num_blocks; i++, block++, data += VDO_BLOCK_SIZE) {
...
/**
 * load_reference_block_set() - After a block waiter has gotten a VIO from the VIO pool, load
 *                              the run of blocks.
 * @waiter: The waiter of the first block to load.
 * @context: The VIO returned by the pool.
 */
static void load_reference_block_run(struct vdo_waiter *waiter, void *context)
{
	struct pooled_vio *pooled = context;
	struct vio *vio = &pooled->vio;
	struct reference_block *block =
	...
	int max_block_count = block->slab->reference_block_count - block_offset;
	int num_blocks = min_t(int, vio->block_count, max_block_count);
	...
	vdo_submit_metadata_vio_with_size(vio, block->slab->ref_counts_origin + block_offset,
					  ...
					  REQ_OP_READ, num_blocks * VDO_BLOCK_SIZE);
}
...
{
	block_count_t i;
	u64 step = slab->allocator->refcount_blocks_per_big_vio;
	...
	slab->active_count = DIV_ROUND_UP(slab->reference_block_count, step);
	for (i = 0; i < slab->reference_block_count; i += step) {
		struct vdo_waiter *waiter = &slab->reference_blocks[i].waiter;

		waiter->callback = load_reference_block_run;
						   ...
						   zone_count_t zone)
{
	...
	/*
	 * Some counts won't divide evenly into maximally sized bios (depending on the slab size
	 * and page size). Figure out how many we're going to need, and then the smallest
	 * allocation we can use if all those bios will get the same size buffer allocated.
	 */
	u32 reference_block_count = vdo_get_saved_reference_count_size(depot->slab_config.slab_blocks);
	u32 refcount_reads_needed = DIV_ROUND_UP(reference_block_count, MAX_BLOCKS_PER_VIO);
	u32 refcount_blocks_per_vio = DIV_ROUND_UP(reference_block_count, refcount_reads_needed);
	...
	*allocator = (struct block_allocator) {
	...
	if (result != VDO_SUCCESS) {
		// non-fatal
		vdo_log_info("failed to allocate large vios for refcount load;"
			     " will fall back to small reads");
	}

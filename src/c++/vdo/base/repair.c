...
static int parse_journal_for_rebuild(struct repair_completion *repair)
{
	...
	result = vdo_allocate(count, __func__,
			      &repair->entries);
	if (result != VDO_SUCCESS)
		return result;
...
static int extract_new_mappings(struct repair_completion *repair)
{
	 ...
	 */
	result = vdo_allocate(repair->entry_count,
...
void vdo_repair(struct vdo_completion *parent)
{
	...
	result = vdo_allocate_flex(page_count, page_completions, __func__, &repair);
	if (result != VDO_SUCCESS) {
	...
	result = vdo_allocate(remaining * VDO_BLOCK_SIZE, __func__,
			      &repair->journal_data);

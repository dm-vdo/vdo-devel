							 ...
							 struct data_vio_compression_status new_status)
{
	...
	actual = atomic_cmpxchg(&data_vio->compression.status, expected,
				replacement);
...
struct data_vio_compression_status advance_data_vio_compression_stage(struct data_vio *data_vio)
{
	for (;;) {
		...
		if (set_data_vio_compression_status(data_vio, status,
						    new_status))
			return new_status;
...
static void attempt_logical_block_lock(struct vdo_completion *completion)
{
	...
	if (!data_vio->write && READ_ONCE(lock_holder->allocation_succeeded)) {
		copy_to_bio(data_vio->user_bio,
			    (lock_holder->vio.data + data_vio->offset));
...
static inline struct data_vio *get_available_data_vio(struct data_vio_pool *pool)
{
	struct data_vio *data_vio =
		list_first_entry(&pool->available, struct data_vio,
				 pool_entry);
...
static int initialize_data_vio(struct data_vio *data_vio, struct vdo *vdo)
{
	...
	initialize_vio(&data_vio->vio, bio, 1, VIO_TYPE_DATA,
		       VIO_PRIORITY_DATA, vdo);
		       ...
		       struct data_vio_pool **pool_ptr)
{
	...
	result = UDS_ALLOCATE_EXTENDED(struct data_vio_pool, pool_size,
				       struct data_vio, __func__, &pool);
	...
	vdo_prepare_completion(&pool->completion, process_release_callback,
			       ...
			       vdo->thread_config.cpu_thread, NULL);
...
{
	bool drained;
	struct data_vio_pool *pool = container_of(state, struct data_vio_pool,
						  state);
			 ...
			 struct vdo_completion *completion)
{
	...
	vdo_start_draining(&pool->state, VDO_ADMIN_STATE_SUSPENDING,
...
static void release_lock(struct data_vio *data_vio, struct lbn_lock *lock)
{
	...
	if (!lock->locked) {
		...
		struct data_vio *lock_holder = vdo_int_map_get(lock_map,
							       lock->lbn);
...
#endif /* INTERNAL */
		uds_funnel_queue_put(pool->queue,
			...
			enum block_mapping_state mapping_state, char *buffer)
{
	...
	if (result != VDO_SUCCESS) {
		uds_log_debug("%s: compressed fragment error %d", __func__,
			      result);
		return result;
	}
...
static void update_block_map(struct vdo_completion *completion)
{
	...
	vdo_set_completion_result(completion,
				  data_vio->decrement_completion.result);
	if (completion->result != VDO_SUCCESS) {
...
static void write_bio_finished(struct bio *bio)
{
	...
	update_metadata_for_data_vio_write(data_vio,
...
void write_data_vio(struct data_vio *data_vio)
{
	...
	do {
		status = get_data_vio_compression_status(data_vio);
		 ...
		 !set_data_vio_compression_status(data_vio, status,
						  new_status));
...
void continue_data_vio_with_block_map_slot(struct vdo_completion *completion)
{
	...
	if (!data_vio->is_zero && (!data_vio->is_trim || data_vio->is_partial)) {
		data_vio_allocate_data_block(data_vio, VIO_WRITE_LOCK,
					     allocate_block,
